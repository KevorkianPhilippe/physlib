...
lemma minkowskiProduct_eq_iff {d : ℕ} {u v : Vector d} (hu : causallyFollows 0 u)
...
lemma sqrt_add_eq_iff {d : ℕ} {u v : Vector d} (hu : causallyFollows 0 u)
    (hv : causallyFollows 0 v) :
    √⟪u + v, u + v⟫ₘ = √⟪u, u⟫ₘ + √⟪v, v⟫ₘ ↔ ⟪u, v⟫ₘ = √⟪u, u⟫ₘ * √⟪v, v⟫ₘ := by
  have hu1 := (causallyFollows_zero_iff.mp hu).1
  have hv1 := (causallyFollows_zero_iff.mp hv).1
  have hsum := minkowskiProduct_add_self u v
  have hnn := (causallyFollows_zero_iff.mp (causallyFollows_zero_add hu hv)).1
  constructor
  · intro h
